...
function App() {
  ...
  const [draggedItemPositionDifference, setDraggedItemPositionDifference] = useState({x: 0, y: 0})
    const items = useRef({
    '1': {
      id: '1',
      posX: 100,
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener("mousemove", (event) => handleBoxPosition(event));
    }
  }, []);

  useEffect(() => {
    if(draggedItemId){
      let newValue = {[draggedItemId]: {
        id: draggedItemId,
  ...
  const onDragItem = useCallback((id: string | null, x: number, y: number) => {
